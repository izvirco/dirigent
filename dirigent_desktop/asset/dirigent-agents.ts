...
mode:"handoff" runs a short launch/feedback script, DO NOT wait or poll. If your workflow needs the child to ask questions or request review, include when and how to use agents.pingParent in your brief. Completion/failure also wakes you automatically, even without a ping. Notifications include agentId/runId and arrive only after that child settles, so send() can safely continue its session. They wake an idle parent or queue as a follow-up behind its current conversation, never steer/interject. The workflow itself finishing does NOT wake you. Call handoff as your only tool call in the batch: Pi ends the turn only when every tool in the batch requests termination. Errors still return to you for recovery. Handoff is not Esc/abort and does not stop children. Feedback via send() in another handoff workflow has the same behavior.
