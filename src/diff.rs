...
mod tests {
    ...
    fn initialize_git(root: &Path) {
        git(root, &["init", "--quiet"]);
        git(root, &["config", "user.name", "Dirigent Test"]);
        git(root, &["config", "user.email", "dirigent@example.invalid"]);
    }

    #[test]
    fn projects_without_vcs_do_not_get_turn_diffs() {
        let root = std::env::temp_dir().join(format!(
            "dirigent-diff-no-vcs-{}-{}",
            std::process::id(),
            fastrand::u64(..)
        ));
        fs::create_dir_all(&root).unwrap();
        assert!(begin_turn(1, "change file", &root).unwrap().is_none());
    ...
    fn previews_active_turn_changes_without_finishing_the_turn() {
        ...
        let active = begin_turn(1, "change file", &root).unwrap().unwrap();
    ...
    #[test]
    fn freezes_jj_modified_and_renamed_files() {
        ...
        let output = Command::new("jj")
            .args(["file", "track", "modified.txt", "old.txt"])
            .current_dir(&root)
            .output()
            .unwrap();
        assert!(output.status.success());
        ...
        let active = begin_turn(1, "change files", &root).unwrap().unwrap();
        ...
        let kinds = turn
            ...
            .collect::<HashMap<_, _>>();
        assert_eq!(kinds["modified.txt"], FileDiffKind::Modified);
        assert_eq!(kinds["new.txt"], FileDiffKind::Renamed);
        fs::remove_dir_all(root).unwrap();
    }

    #[test]
    fn scopes_git_turn_diffs_to_the_project_directory() {
        let repository = std::env::temp_dir().join(format!(
            "dirigent-scoped-git-diff-{}-{}",
            std::process::id(),
            fastrand::u64(..)
        ));
        ...
        fs::write(project.join("inside.txt"), "before\n").unwrap();
        fs::write(repository.join("outside.txt"), "before\n").unwrap();
        git(&repository, &["add", "."]);
        git(&repository, &["commit", "--quiet", "-m", "initial"]);

        let active = begin_turn(1, "change project", &project).unwrap().unwrap();
        fs::write(project.join("inside.txt"), "after\n").unwrap();
        fs::write(repository.join("outside.txt"), "after\n").unwrap();
        let turn = finish_turn(active, &project, TurnDiffStatus::Completed);

        assert_eq!(turn.files.len(), 1);
        assert_eq!(turn.files[0].path, "inside.txt");
        fs::remove_dir_all(repository).unwrap();
    }

    #[test]
    fn preserves_preexisting_git_changes_as_the_turn_baseline() {
        let root = std::env::temp_dir().join(format!(
            "dirigent-dirty-git-diff-{}-{}",
            std::process::id(),
            fastrand::u64(..)
        ));
        fs::create_dir_all(&root).unwrap();
        ...
        git(&root, &["add", "file.txt"]);
        git(&root, &["commit", "--quiet", "-m", "initial"]);
        fs::write(root.join("file.txt"), "before turn\n").unwrap();

        let active = begin_turn(1, "change dirty file", &root).unwrap().unwrap();
        fs::write(root.join("file.txt"), "after turn\n").unwrap();
        let turn = finish_turn(active, &root, TurnDiffStatus::Completed);

        assert_eq!(turn.files.len(), 1);
        assert_eq!(turn.files[0].old_text.as_deref(), Some("before turn\n"));
        assert_eq!(turn.files[0].new_text.as_deref(), Some("after turn\n"));
        fs::remove_dir_all(root).unwrap();
    }
