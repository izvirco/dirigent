...
impl Dirigent {
    ...
    fn render_assistant_segment_shell(
        ...
        div()
            .id(("assistant-message-segment", index * 10_000 + segment_index))
            .relative()
            .w_full()
            .pr_3()
            .border_r_4()
            ...
            .child(
                div()
                    ...
                    .min_w(px(0.0))
                    .flex_1()
                    .when(top_gap, |element| element.pt_2())
                    .child(content),
            )
            .when(last, |element| {
                element.child(self.render_message_actions(message, index, actions_visible, cx))
            })
            .into_any_element()
    }

    #[allow(clippy::too_many_arguments)]
    pub(super) fn render_assistant_text_segment(
        &self,
        message: &Message,
        index: usize,
        segment_index: usize,
        ...
        let content = self.render_selectable_text(
            format!("assistant-message-text-{index}-{segment_index}"),
            text,
            &[],
            cx,
        );
    ...
    #[allow(clippy::too_many_arguments)]
    pub(super) fn render_assistant_markdown_segment(
        &self,
        message: &Message,
        index: usize,
        segment_index: usize,
        ...
        let content = self.render_markdown_block_segment(
            block,
            original_block,
