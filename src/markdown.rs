...
mod tests {
    ...
    fn parses_tables_with_alignment_and_inline_formatting() {
        ...
        assert_eq!(table.rows[0][0].text, "alpha");
        assert!(table.rows[0][0].spans[0].style.strong);
        assert!(table.rows[0][1].spans[0].style.code);
        assert!(table.rows[0][2].spans[0].style.emphasis);
    }

    #[test]
    fn parses_nested_lists_and_code_blocks() {
        let list_document = parse_markdown("1. first\n    - nested\n");
        let MarkdownBlock::List { start, items } = &list_document.blocks[0] else {
            panic!("expected a list")
        };
        assert_eq!(*start, Some(1));
        assert!(matches!(items[0][1], MarkdownBlock::List { .. }));

        let code_document = parse_markdown("```rust\nfn main() {}\n```\n");
        let MarkdownBlock::CodeBlock { language, code } = &code_document.blocks[0] else {
    ...
    #[test]
    fn removes_markers_and_keeps_inline_styles() {
        let document = parse_markdown("A **bold _and italic_** [link](https://example.com).");
        let MarkdownBlock::Paragraph(text) = &document.blocks[0] else {
            panic!("expected a paragraph")
        };
        assert_eq!(text.text, "A bold and italic link.");
        assert!(text.spans.iter().any(|span| span.style.strong));
        assert!(
            text.spans
                .iter()
        ...
        assert!(
            text.spans
                .iter()
                .any(|span| { span.style.link.as_deref() == Some("https://example.com") })
        );
    }
}
